...
var apiCmdDict = {
  ...
  '^get disk (da[0-9]) iostat krs$' : 'iostat -d -x | awk \'/^%1/ { printf "%s", $4 }\'',
  '^get disk (da[0-9]) iostat kws$' : 'iostat -d -x | awk \'/^%1/ { printf "%s", $5 }\'',
  '^get disk (da[0-9]) iostat qlen$' : 'iostat -d -x | awk \'/^%1/ { printf "%s", $10 }\'',
