...
var apiCmdDict = {
  ...
  '^get partition (da[0-9])p([1-9])$': 'sysctl -n kern.geom.conftxt | awk \'/%1p%2/ { printf "[ %s, \\"%s\\", \\"%s\\" ]", $4, $11, $13 }\'',
  '^get partition (da[0-9])p([1-9]) size$': 'sysctl -n kern.geom.conftxt | awk \'/PART %1p%2/ { printf "%s", $4 }\'',
  '^get partition (da[0-9])p([1-9]) type$': 'sysctl -n kern.geom.conftxt | awk \'/PART %1p%2/ { printf "\\"%s\\"", $11 }\'',
  '^get partition (da[0-9])p([1-9]) filesystem$': 'sysctl -n kern.geom.conftxt | awk -F\'[ /]\' \'/PART %1p%2/ { getline; printf "\\"%s\\"", $3 }\'',
  '^get partition (da[0-9])p([1-9]) filesystem label$': 'sysctl -n kern.geom.conftxt | awk -F\'[ /]\' \'/PART %1p%2/ { getline; printf "\\"%s\\"", $4 }\'',
  ...
  '^get system mail raw$' : 'mailx -Hu `awk -F\': *\' \'/^root:/ { print $2 }\' /etc/aliases`',
  '^get system mail ([0-9]+) raw$' : 'echo "%1" | mailx -Nu `awk -F\': *\' \'/^root:/ { print $2 }\' /etc/aliases`',
