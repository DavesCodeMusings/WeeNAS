...
var apiCmdDict = {
  ...
  '^get users$': '/usr/local/bin/pdbedit -L | /usr/bin/awk -F: \'BEGIN { print "[" } { printf "%s\\"%s\\"", (NR==1)?"":",\\n", $1 } END { printf "\\n]" }\'',
  '^get users flags$' : '/usr/local/bin/pdbedit -L -v | /usr/bin/awk -F\': *\' \'BEGIN { count=0; print "{" } /---------------/ { getline; printf "%s\\"%s\\" : ", (++count==1)?"":",\\n", $2; getline; getline; printf "\\"%s\\"", $2 } END { printf "\\n}" }\'',
  ...
  '^get datetime$': 'echo "\\"$(date -u +%Y-%m-%dT%H:%MZ)\\""',
  ...
  '^get disks$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'BEGIN { i=0; printf "[ " } /DISK/ { printf "%s\\"%s\\"", (++i==1)?"":", ", $3 } END { printf " ]" }\'',
  '^get disk (da[0-9]|mmcsd0)$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'BEGIN { printf "[ " } /PART %1/ { printf "%s\\"%s\\"",(++count==1)?"":", ",  $3 } END { printf " ]" }\'',
  '^get disk (da[0-9]|mmcsd0) info$': 'dmesg | /usr/bin/awk \'BEGIN { printf "[ " } /^%1:/ { printf "\\""; for(i=2;i<=NF;i++) printf "%s%s", (i==2)?"":" ", $i; printf "\\"" } END { printf " ]" }\' | sed \'s/""/", "/g\'',
  ...
  '^get disk (da[0-9]|mmcsd0) size$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/DISK %1/ { printf "%s", $4 }\'',
  '^get partitions (da[0-9]|mmcsd0)$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'BEGIN { printf "[ " } /PART %1/ { printf "%s\\"%s\\"",(++count==1)?"":", ",  $3 } END { printf " ]" }\'',
  '^get partition (da[0-9])p([1-9])$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/%1p%2/ { printf "[ %s, \\"%s\\", \\"%s\\" ]", $4, $11, $13 }\'',
  '^get partition (da[0-9])p([1-9]) size$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/PART %1p%2/ { printf "%s", $4 }\'',
  '^get partition (da[0-9])p([1-9]) type$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/PART %1p%2/ { printf "\\"%s\\"", $11 }\'',
  '^get partition (da[0-9])p([1-9]) filesystem$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk -F\'[ /]\' \'/PART %1p%2/ { getline; printf "\\"%s\\"", $3 }\'',
  '^get partition (da[0-9])p([1-9]) filesystem label$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk -F\'[ /]\' \'/PART %1p%2/ { getline; printf "\\"%s\\"", $4 }\'',
  '^get filesystems$': 'df -g | /usr/bin/awk \'BEGIN { printf "[ "; count=0 } /^\\/dev/ { printf "%s\\n", (++count==1)?"":", "; printf "[ \\"%s\\", %s, %s, %s, \\"%s\\", \\"%s\\" ]", $1, $2, $3, $4, $5, $6 } END { printf "\\n]" }\'',
  '^get filesystems ufs$': 'df -g | /usr/bin/awk \'BEGIN { printf "[ "; count=0 } /^\\/dev\\/ufs/ { printf "%s\\n", (++count==1)?"":", "; printf "[ \\"%s\\", %s, %s, %s, \\"%s\\", \\"%s\\" ]", $1, $2, $3, $4, $5, $6 } END { printf "\\n]" }\'',
  '^get filesystem ufs ([a-z]+)$': 'df -g | /usr/bin/awk \'/^\\/dev\\/ufs\\/%1/ { printf "[ \\"%s\\", %s, %s, %s, \\"%s\\", \\"%s\\" ]", $1, $2, $3, $4, $5, $6 }\'',
  '^get filesystem ufs ([a-z]+) size$': 'df -g | /usr/bin/awk \'/^\\/dev\\/ufs\\/%1/ { printf "%s", $2 }\'',
  '^get filesystem ufs ([a-z]+) used$': 'df -g | /usr/bin/awk \'/^\\/dev\\/ufs\\/%1/ { printf "%s", $3 }\'',
  '^get filesystem ufs ([a-z]+) available$': 'df -g | /usr/bin/awk \'/^\\/dev\\/ufs\\/%1/ { printf "%s", $4 }\'',
  '^get filesystem ufs ([a-z]+) capacity$': 'df -g | /usr/bin/awk \'/^\\/dev\\/ufs\\/%1/ { printf "\\"%s\\"", $5 }\'',
  '^get filesystem ufs ([a-z]+) mountpoint$': 'df -g | /usr/bin/awk \'/^\\/dev\\/ufs\\/%1/ { printf "\\"%s\\"", $6 }\'',
