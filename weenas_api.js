...
var apiCmdDict = {
  ...
  '^get users$': '/usr/local/bin/pdbedit -L -v | /usr/bin/awk -F\': *\' \'BEGIN { count=0; print "{" } /---------------/ { getline; printf "%s\\"%s\\" : ", (++count==1)?"  ":",\\n  ", $2; getline; getline; printf "\\"%s\\"", $2 } END { printf "\\n}" }\'',
  ...
  '^get disks$': '/sbin/sysctl -n kern.geom.conftxt | awk \'BEGIN { printf "{"; count=0 } /^0 DISK/ { printf "%s\\"%s\\": { \\"size\\": %s, ", (++count==1)?"\\n  ":",\\n  ", $3, $4; getline; printf "\\"label\\": \\"%s\\" }", ($2=="LABEL")?$3:"" } END { printf "\\n}" }\'',
  '^get disk (da[0-9]|mmcsd0)$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'BEGIN { printf "[ " } /^[12] PART %1/ { printf "%s\\"%s\\"",(++count==1)?"":", ",  $3 } END { printf " ]" }\'',
  ...
  '^get disk (da[0-9]|mmcsd0) size$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/^0 DISK %1/ { printf "%s", $4 }\'',
  '^get partition ((?:da[0-9]|mmcsd0)[ps][1-9][a-h]*)$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/%1/ { printf "[ %s, \\"%s\\", \\"%s\\"", $4, $11, $13; getline; printf ", \\"%s\\" ]", $3 }\'',
  '^get partition ((?:da[0-9]|mmcsd0)[ps][1-9][a-h]*) size$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/^[12] PART %1/ { printf "%s", $4 }\'',
  '^get partition ((?:da[0-9]|mmcsd0)[ps][1-9][a-h]*) type$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk \'/^[12] PART %1/ { printf "\\"%s\\"", $11 }\'',
  '^get partition ((?:da[0-9]|mmcsd0)[ps][1-9][a-h]*) filesystem$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk -F\'[ /]\' \'/^[12] PART %1/ { getline; printf "\\"%s\\"", $3 }\'',
  '^get partition ((?:da[0-9]|mmcsd0)[ps][1-9][a-h]*) filesystem label$': '/sbin/sysctl -n kern.geom.conftxt | /usr/bin/awk -F\'[ /]\' \'/^[12] PART %1/ { getline; printf "\\"%s\\"", $4 }\'',
