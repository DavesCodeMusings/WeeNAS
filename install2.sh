...
dialog --no-lines --backtitle "$BACKTITLE" --title "Hostname" \
 --form "If you'd like to set a new DNS identity for your WeeNAS device (e.g. weenas.mydomain.com) type it below. Or select cancel to keep the default." 10 60 1 "hostname:" 0 0 "$(hostname)" 0 11 42 42 2>$TMP_FILE
...
while [ $(geom disk status | grep -c da) -ne 1 ]; do
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --yes-label "Rescan" --no-label "Exit" \
   --yesno "During installation, there should be one and only one flash drive plugged in. This device will be dedicated to data storage. Remove all other USB storage devices from the system before proceeding." 0 0
  [ $? -eq 0 ] || exit 1
done
...
if ! service ntpd status >/dev/null 2>&1; then
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
   --infobox "Configuring NTP time synchronization.\n\n[ ] Enable NTP.\n[ ] Set timezone.\n[ ] Start NTP service." $BOX_H $BOX_W
  ...
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
   --infobox "Configuring NTP time synchronization.\n\n[x] Enable NTP.\n[x] Set Timezone.\n[x] Start NTP service." $BOX_H $BOX_W
else
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
   --infobox "Configuring NTP time synchronization.\n\n[s] Enable NTP.\n[s] Set timezone.\n[s] Start NTP service.\n\nAll tasks skipped, because ntpd is already running." $BOX_H $BOX_W
fi
...
if [ "$NEW_HOSTNAME" != ""]; then
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 --infobox "Setting new hostname.\n\n[ ] $NEW_HOSTNAME" 6 $BOX_W
...
pkg info node12 >>$LOGFILE 2>&1 || pkg install -y samba410 >>$LOGFILE 2>&1
dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
 --infobox "Installing required packages. Please be patient, this can take some time.\n\n[x] pkg the FreeBSD package manager.\n[x] Node.js JavaScript runtime.\n[x] Install Samba SMB/CIFS file sharing." $BOX_H $BOX_W
...
#  mount | grep homefs >/dev/null && { echo "Cowardly refusing to overwrite a mounted filesystem."; mount | grep homefs; exit 1; }
  ...
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
   --infobox "Configuring $DEVICE.\n\n[x] Write new GPT partition scheme.\n[x] Create FreeBSD-UFS filesystem." 8 $BOX_W
fi
...
if ! mount | grep homefs >>$LOGFILE 2>&1; then
  ...
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
   --infobox "Preparing homefs.\n\n[s] Add to /etc/fstab.\n[s] Check filesystem.\n[s] Mount filesystem.\n\nAll tasks skipped, because homefs is already mounted." $BOX_H $BOX_W
fi

# Configure Samba
TITLE="Samba Configuration"
echo "$TITLE" >>$LOGFILE
if ! service samba_server status >/dev/null 2>&1; then
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
   --infobox "Setting up Samba file sharing.\n\n[ ] Create basic configuration file.\n[ ] Start services." $BOX_H $BOX_W
...
echo "$TITLE" >>$LOGFILE
  ...
  dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
   --infobox "Setting up WeeNAS administration. Please be patient, this can take some time.\n\n[x] Create TLS certificate.\n[x] Install service.\n[x] Start service." $BOX_H $BOX_W

# Set up users.
TITLE="User Accounts"
echo "$TITLE" >>$LOGFILE
dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
 --infobox "Configuring users.\n\n[ ] Create shared group.\n[ ] Lock backdoor toor account.\n[ ] Enable freebsd user as WeeNAS admin." $BOX_H $BOX_W
...
dialog --no-lines --backtitle "$BACKTITLE" --title "$TITLE" --sleep 2 \
 --infobox "Configuring users.\n\n[x] Create shared group.\n[x] Lock backdoor toor account.\n[x] Enable freebsd user as WeeNAS admin." $BOX_H $BOX_W
...
while [ $PW_SET -ne 1 ]; do
  ...
  dialog --no-lines --title "$TITLE" --insecure \
   --passwordform "Enter a new password.\nUse up/down arrow keys to move between fields." $BOX_H $BOX_W 2 "New Password:" 1 6 "" 1 21 32 32 "Confirm Password:" 2 2 "" 2 21 32 32 2>$TMP_FILE
  if [ $? -eq 0 ]; then
    umask $UMASK_SAVE
